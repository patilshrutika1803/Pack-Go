...
export default function PlannerPage() {
  ...
  const handleSubmit = async (query) => {
    ...
    try { const response = await fetch('/plan/stream', { method: 'POST', headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) }, body: JSON.stringify({ question: query, thread_id: crypto.randomUUID(), remember_me: true }) }); if (!response.ok) throw new Error('Unable to start your plan.')
      ...
      while (true) { const { value, done } = await reader.read(); if (done) break; buffer += decoder.decode(value, { stream: true }); const lines = buffer.split('\n\n'); buffer = lines.pop(); for (const chunk of lines) { const line = chunk.trim(); if (!line.startsWith('data: ')) continue; try { const event = JSON.parse(line.slice(6)); const { status, agent, message, data } = event; if (status === 'running' && agent && agent !== 'System') { setCurrentAgent(agent); setAgents(prev => prev.includes(agent) ? prev : [...prev, agent]) } if (status === 'done') { setCurrentAgent(null); if (data?.intent === 'general_chat') { setMessages(prev => [...prev, { role: 'assistant', content: data.response || '', sources: data.sources || [], id: Date.now() }]) } else setPlan(data); setLoading(false) } if (status === 'error') { setError(message); setLoading(false) } } catch { /* Ignore incomplete SSE chunks. */ } } }
